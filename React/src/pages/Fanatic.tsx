...
function Fanatic() {
  ...
  const isOutOfTries =
    triesInfo?.remaining_tries_game === 0 ||
    triesInfo?.remaining_tries_today === 0;
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="w-full flex justify-between items-center px-10 py-8">
            <div className="flex items-center gap-3">
              <span className="font-anton text-5xl text-primary">
                {bestTry?.highest_similarity ?? 0}%
              </span>
                ...
                <Countdown
                  ...
                  renderer={(props) =>
                    props.completed ? null : (
                      <>Next riddle in: {formatTime(props)}</>
                    )
                  }
                />
              )}
          ...
          <Dialog isOpen={dialogOpen} className="max-w-2xl mx-auto">
            <div className="flex flex-col gap-3 text-center items-center">
              <h2
                className={`text-3xl font-anton ${submitError ? 'text-red-500' : 'text-black'} mb-2`}
              >
                {handleSimilarityText(submitResult?.similarity_score ?? null)}
              </h2>
              {submitError ? (
                <p className="text-black font-lato text-lg text-center">
                  There was an error submitting your guess. Please try again
                  later.
                </p>
              ) : (
                <>
          ...
          <div className="flex flex-col gap-6 text-left">
            <div className="max-w-3xl mx-auto">
              ...
              <p className="font-lato text-base mt-2">
                Log in, open the active Fanatic challenge, read the current
                clues, and submit your guess for the person, place, or thing
                behind the category.
              </p>
              <p className="font-lato text-base mt-2">
                Keep checking back as the game progresses, because new clues
                unlock over time and the page shows when the next riddle will
                appear.
              </p>
            </div>
            <div className="max-w-3xl mx-auto">
              ...
              <p className="font-lato text-base mt-2">
                You get 1 attempt every 24 hours and up to 3 attempts total for
                the whole game.
              </p>
              <p className="font-lato text-base mt-2">
              ...
              <p className="font-lato text-base mt-2">
                Guessing earlier is better because the score multiplier
                decreases as days pass, and your best similarity plus remaining
                attempts are shown at the top.
              </p>
            </div>
          </div>
        </div>
